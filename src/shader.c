...
static void		shader_perror(GLuint shader_id, GLint log_length)
{
...
static void		shader_program_perror(GLuint program_id, GLint log_length)
{
	char	*log = malloc(sizeof(*log) * log_length + 1);
...
static GLuint	shader_link(GLuint vertex_shader_id, GLuint fragment_shader_id)
{
	GLuint	program_id;
	GLint	status;
	GLint	log_length;
...
GLuint			shader_load(const char *vertex_file, const char *fragment_file)
{
	...
	if (vertex_shader_id != 0)
	{
		...
		{
			program_id = shader_link(vertex_shader_id, fragment_shader_id);
